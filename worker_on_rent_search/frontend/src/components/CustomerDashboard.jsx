...
export default function CustomerDashboard() {
    ...
    // Auto-trigger search if ?q= param present (e.g. navigating from Categories page)
    useEffect(() => {
        const q = searchParams.get('q')
        if (q && q.trim()) {
            setSearchQuery(q.trim())
            setIsSearchMode(true)
            runSearch(q.trim(), mapState?.lat, mapState?.lng)
            setTimeout(() => document.getElementById('workers-section')?.scrollIntoView({ behavior: 'smooth' }), 400)
        }
    }, [searchParams.get('q')])
            ...
            <div className='w-full bg-white py-8'>
                <div className='max-w-7xl mx-auto px-4 md:px-6'>
                    <div className='flex items-center justify-between mb-5'>
                        ...
                        <button onClick={() => navigate('/categories')} className='text-sm text-[#ff4d2d] font-semibold'>View all →</button>
                    </div>
                    {/* Mobile circles */}
            ...
            <div id='workers-section' className='w-full max-w-7xl mx-auto px-4 md:px-6 py-6 md:py-10'>
                ...
                <div className='flex items-center justify-between mb-4 flex-wrap gap-2'>
                    ...
                    <div className='flex items-center gap-3'>
                        ...
                        {!isSearchMode && (
                            <button onClick={() => navigate('/workers')} className='text-sm text-[#ff4d2d] font-semibold'>View all workers →</button>
                        )}
                    </div>
                </div>
